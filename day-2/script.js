...
*/
const puzzleInput = `9-10 m: mmmmnxmmmwm
6-8 w: wpwwhxnv
4-6 n: trwpnnnvq
...
const passwords = puzzleInput.split('\n').map((el) => {
    const splitted = el.split(' ');
    return {
        minChar: Number(splitted[0].split('-')[0]),
        maxChar: Number(splitted[0].split('-')[1]),
        char: splitted[1][0],
        password: splitted[2],
    };
});

const validPasswords = (passArray) => {
    let validCounterPolicy1 = 0;
    let validCounterPolicy2 = 0;
    passArray.forEach((object) => {
        const { minChar, maxChar, char, password } = object;
        // console.log(minChar, maxChar, char, password);
        let count = 0;
        let count2 = 0;
        ...
        if (count >= minChar && count <= maxChar) validCounterPolicy1++;

        // second part
        if (password.charAt(minChar - 1) === char) count2++;
        if (password.charAt(maxChar - 1) === char) count2++;

        if (count2 === 1) validCounterPolicy2++;
    });

    return [validCounterPolicy1, validCounterPolicy2];
};
